...
server.post('/tweets', async (req, res, next) => {
  try {
    const query = await db.run(`INSERT INTO Tweets (userId, text, createAt) values ("${req.body.userId}", "${req.body.text}", "${Date.now()}")`);
...
server.get('/tweets', async (req, res, next) => {
  try {
    const tweets = await db.all(`SELECT Tweets.id, Tweets.text, Tweets.createAt, Users.name from Tweets INNER JOIN Users ON Users.id=Tweets.userId ORDER BY Tweets.createAt DESC`);
