...
server.post('/tweets', async (req, res, next) => {
  try {
    const tweet = await db.run(`INSERT INTO Tweets (userId, text) values ("${req.body.userId}", "${req.body.text}")`);
    console.log(tweet);
    res.send(200);
  } catch (err) {
    next(err)
...
server.get('/tweets', async (req, res, next) => {
  try {
    const tweets = await db.all(`SELECT Tweets.text, Users.name from Tweets INNER JOIN Users ON Users.id=Tweets.userId`);
    res.send(tweets);
  } catch (err) {
    next(err)
  }
